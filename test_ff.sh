...
ROOT_PATH=/home/law/HDD/serein/Dataset/FF++/
FAKE_ROOT_PATH=""
DATASET_NAME=FF++
SAVE_TXT=./output_more_epoch/results/FF++_metrics_rebuttal.txt
INPUT_SIZE=224
BATCH_SIZE=24
FAKE_INDEXES=(1 2 3 4)
...
do
  ...
  nohup python train.py --root_path ${ROOT_PATH} --fake_root_path '' --model_name ${MODEL_NAME} \
                  --input_size ${INPUT_SIZE} --batch_size ${BATCH_SIZE} --device_id ${DEVICE_ID} --is_test \
                  --model_path ${MODEL_PATH} --is_crop --fake_indexes ${FAKE_INDEX} \
                  --dataset_name ${DATASET_NAME} --save_txt ${SAVE_TXT} --embedding_size ${EMBEDDING_SIZE} > output_FF++_test_convB${FAKE_INDEX}.log 2>&1
done
