...
from plot.plot_utils import plot_score_distribution
import numpy as np
...
def calculate_acc(y_true, y_pred, thres):#计算准确率
    r_acc = accuracy_score(y_true[y_true==0], y_pred[y_true==0] > thres)#计算真实值为0的准确率
    f_acc = accuracy_score(y_true[y_true==1], y_pred[y_true==1] > thres)#计算伪造样本为1的准确率
    acc = accuracy_score(y_true, y_pred > thres)#计算总体准确率
    return r_acc, f_acc, acc    #返回真实值为0的准确率，伪造样本为1的准确率，总体准确率
...
def eval_model(model, epoch, eval_loader, tokenizer, is_save=True, threshold=0.5, alpha=0.5,
    ...
    with torch.no_grad():
        for i, (img, type,label) in enumerate(eval_process):
            ...
            label = label.cuda().float() # 标签张量也需要移动到GPU
            # label.extend(label.flatten().tolist())
            # 前向传播，获取模型的预测结果和嵌入向量
            ...
            # # y_score.extend(torch.sigmoid(y_pred).cpu().numpy().tolist())
            # y_score.extend(torch.sigmoid(y_pred).detach().cpu().numpy().tolist())
            # 对预测结果应用 Softmax 函数，将输出转换为概率分布
            # y_pred = nn.Softmax(dim=1)(y_pred)
            # y_pred.extend(y_pred.sigmoid().flatten().tolist())
            # 计算总损失，包括分类损失和对比损失
            ...
            # outputs_fake.append(y_pred[:, 1]) # 收集伪造类别的概率
            # 存储当前批次的真实标签
            labels.append(label)
            # # 计算当前批次的准确率
            # acc = (torch.max(y_pred.detach(), 1)[1] == label).sum().item() / batch_size
            # acc = accuracy_score(np.array(label), np.array(y_pred > 0.5))
            acc = accuracy_score(np.array(label.cpu()), np.array(probs.detach().cpu()) > 0.5)
            # 更新损失平均值
    ...
    if save_txt is not None:
        return (binary_acc, recall, precision, f1, fnr), pre_dict

    # 否则返回平均准确率
    return acc_avg, pre_dict
...
def train_model(model, criterion, optimizer,tokenizer, epoch, scaler=None, alpha=0.5):
    ...
    for i, (input, types, label) in enumerate(training_process):
        # #返回的是image_list: 包含原始图像和重建图像的列表 [image, rec_image]
        #         type: 字符串类型标签，要么是 'real'（真实图像）要么是 'fake'（伪造/生成图像）
        #         label: 数字标签或者 one-hot 编码标签（取决于 self.is_one_hot 设置）
        ...
        if scaler is None:
            ...
            # 计算总损失，包括分类损失和对比损失
            # y_true.extend(label.cpu().numpy().tolist())
            # # y_score.extend(torch.sigmoid(preds).cpu().numpy().tolist())
            # y_score.extend(torch.sigmoid(preds).detach().cpu().numpy().tolist())
            # loss = (1-alpha) * criterion(y_pred, label) + alpha * contrastive_loss(embeddings, label)
            ...
            # acc = accuracy_score(np.array(label), np.array(preds) > 0.5)
            # acc = accuracy_score(np.array(label.cpu()), np.array(preds.detach().cpu()) > 0.5)
            acc = accuracy_score(np.array(label.cpu()), np.array(probs.detach().cpu()) > 0.5)
            # acc = accuracy_score(np.array(y_true), np.array(y_score) > 0.5)
            # 更新损失平均值
            losses.update(loss.item(), batch_size)
            # 更新准确率平均值
            accuracies.update(acc, batch_size)
            ...
            with autocast():
                preds, preds2 = model(tokenizer,input , types, None)
                # preds.extend(preds.sigmoid().flatten().tolist())
                # y_score.extend(torch.sigmoid(preds).cpu().numpy().tolist())
                # 计算总损失，包括分类损失和对比损失
                # loss = (1-alpha) * criterion(preds, label) + alpha * criterion(preds2, label)
                # loss = criterion(preds, label) + criterion(preds2, label)
                # loss = criterion(np.array(y_score), np.array(y_true))
...
if __name__ == '__main__':
    ...
    vis_dir = f"./output_DR-inctro_transform-drct_dire_final/{args.dataset_name}/vis_results/{args.fake_indexes.replace(',', '_')}/" 
                
    

    # --- 【新增】创建可视化结果保存目录 ---
    # vis_dir = writeFile.replace('/logs', '/vis_results')
    ...
    if is_train:
        ...
        # --- 【修改】获取初始最佳准确率时只取 acc ---
        # initial_eval_result, _ = eval_model(model, args.epoch_start - 1, eval_loader, tokenizer, is_save=False)
        ...
        for epoch in range(args.epoch_start, args.num_epochs):
            ...
            if epoch % 1 == 0 or epoch == args.num_epochs - 1:
                acc, pre_dict = eval_model(model, epoch, eval_loader, tokenizer, alpha=args.alpha)
                ...
                if best_acc < acc:
                    ...
                    # --- 【新增】在保存最佳模型时，调用绘图函数 ---
                    plot_score_distribution(pre_dict, vis_dir,epoch)
                    # --- 【新增结束】 ---
                # save_path = '{}/{}_acc{:.4f}.pth'.format(store_name, epoch, acc)
                # if torch.cuda.device_count() > 1:
                #         torch.save(model.module.state_dict(), save_path)
        ...
        # 评估模型在测试集上的性能
        out_metrics, pre_dict = eval_model(model, epoch_start, test_loader, tokenizer, is_save=False, threshold=args.threshold, save_txt=args.save_txt)
        # --- 【新增】在测试模式下也可以选择性地绘图 ---
        plot_score_distribution(pre_dict, vis_dir)
        # --- 【新增结束】 ---
        # 打印测试总耗时
